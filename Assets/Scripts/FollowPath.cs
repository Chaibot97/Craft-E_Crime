...
public class FollowPath : MonoBehaviour
{
    ...
    public void Update()
    {
        Debug.DrawLine(transform.position + (Vector3.up * 0.1f), transform.position + (Vector3.up * 0.2f) + (transform.forward * 3),Color.red);
        //Validate there is a path with a point in it
        if (pointInPath == null || pointInPath.Current == null)
        {
            return; //Exit if no path is found
        }
        var rotation = Quaternion.LookRotation(pointInPath.Current.position- transform.position);
        transform.rotation = Quaternion.RotateTowards(transform.rotation, rotation, Time.deltaTime * 480);
